...
/**
 * Integration tests for MCP client OAuth2 integration. Tests that mcp-client
 * sends OAuth2 tokens to mcp-server. Uses TestContainers to start backend
 * OAuth2 server.
 *
...
class McpClientOAuth2IntegrationTest extends AbstractOAuth2IntegrationTest {
	...
	void shouldUseClientCredentialsForMcpServerCalls() {
		...
		ClientRegistration clientRegistration = ClientRegistration.withRegistrationId("mcp-client-to-server")
				.clientId("test-mcp-client").clientSecret("test-secret")
				.authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS)
				.tokenUri("http://localhost:9090/oauth2/token").build();

		OAuth2AccessToken accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER,
				"test-mcp-access-token", Instant.now(), Instant.now().plusSeconds(3600));

		OAuth2AuthorizedClient authorizedClient = new OAuth2AuthorizedClient(clientRegistration, "mcp-client",
				accessToken);
		...
		// Note: Actual MCP client instantiation would require the MCP server to be
		// running
		assertThat(mcpClientManager).isNotNull();
	}
	...
	void shouldCreateMcpSyncClients() {
		...
		// Note: In test environment without actual MCP servers, this may return empty
		// list
