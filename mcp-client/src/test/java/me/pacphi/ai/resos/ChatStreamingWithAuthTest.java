...
/**
 * Integration tests for chat streaming with authentication. Tests that chat
 * endpoints require authentication and work correctly when authenticated. Uses
 * TestContainers to start backend OAuth2 server.
 *
...
class ChatStreamingWithAuthTest extends AbstractOAuth2IntegrationTest {
	...
	void shouldDenyChatAccessWithoutAuthentication() throws Exception {
		...
		mockMvc.perform(post("/api/v1/resos/stream/chat").with(csrf()) // Add CSRF token
				.contentType(MediaType.APPLICATION_JSON).content("{\"question\":\"Hello\"}"))
	...
	void shouldHandleChatServiceErrors() throws Exception {
		...
		mockMvc.perform(post("/api/v1/resos/stream/chat")
				.with(oidcLogin().idToken(token -> token.claim("sub", "test-user"))).with(csrf()) // Add CSRF token for
																									// POST request
				.contentType(MediaType.APPLICATION_JSON).content("{\"question\":\"Cause error\"}"))
	...
	void shouldRequireValidRequestBody() throws Exception {
		...
		// Then: Returns 200 OK (endpoint accepts request, validation happens in service
		// layer)
		mockMvc.perform(post("/api/v1/resos/stream/chat")
				.with(oidcLogin().idToken(token -> token.claim("sub", "test-user"))).with(csrf()) // Add CSRF token for
																									// POST request
