...
/**
 * Security configuration for the MCP Client (frontend) application. Uses OAuth2
 * login with Authorization Code + PKCE flow. WebMVC-based (servlet stack)
 * configuration.
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		return http.authorizeHttpRequests(authorize -> authorize
				...
				.cors(cors -> cors.configurationSource(corsConfigurationSource())).csrf(csrf -> csrf
	...
	public AuthenticationFailureHandler authenticationFailureHandler() {
		return (request, response, exception) -> {
			...
			response.getWriter()
					.write("<html><body><h1>OAuth2 Authentication Failed</h1>" + "<p>Error: " + exception.getMessage()
							+ "</p>" + "<p><a href='/oauth2/authorization/frontend-app'>Try again</a></p>"
							+ "</body></html>");
		};
	}
	...
	public CorsConfigurationSource corsConfigurationSource() {
		...
		configuration.setAllowedOrigins(
				Arrays.asList("http://localhost:8081", "http://localhost:3000", "http://localhost:5173" // Vite dev
																										// server
				));
