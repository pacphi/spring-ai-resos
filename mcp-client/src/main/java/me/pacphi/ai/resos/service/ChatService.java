...
public class ChatService {
	...
	public ChatService(ChatModel chatModel, ChatMemory chatMemory, McpSyncClientManager syncClientManager) {
	 ...
	 *            Callback for errors
	 */
	public void streamResponseToQuestion(String question, Consumer<String> onToken, Runnable onComplete,
		...
		try {
			...
			var stream = constructRequest(question).toolCallbacks(provider.getToolCallbacks()).stream().content();
	...
	private ChatClient.ChatClientRequestSpec constructRequest(String question) {
		return chatClient.prompt().system(buildSystemPrompt()).user(question);
	}
