...
/**
 * Debug filter to log incoming request cookies and session info. This helps
 * diagnose session/cookie issues.
 */
@Component
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (uri.startsWith("/api/")) {
			...
			if (cookies != null && cookies.length > 0) {
				log.info("Cookies present: {}", Arrays.stream(cookies).map(c -> {
			...
			log.info("Requested Session ID: {}, valid: {}, from cookie: {}",
					sessionIdPartial != null ? sessionIdPartial : "[null]", request.isRequestedSessionIdValid(),
			...
			if (existingSession != null) {
				...
				String existingIdPartial = existingId.length() > 8
						...
						: existingId;
				log.info("Server session exists: {}, matches request: {}", existingIdPartial,
			...
			log.info("Cookie header: {}",
					cookieHeader != null ? "[present, length=" + cookieHeader.length() + "]" : "[null]");
		}
