...
/**
 * Integration tests for Backend API OAuth2 client configuration. Tests that the
 * mcp-server includes OAuth2 tokens when calling the backend API.
 */
@SpringBootTest
@ActiveProfiles("test")
class BackendApiOAuth2ClientTest {
	...
	void shouldIncludeOAuth2TokenInBackendCalls() {
		...
		ClientRegistration clientRegistration = ClientRegistration.withRegistrationId("mcp-server")
				.clientId("test-client").clientSecret("test-secret")
				.authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS)
				.tokenUri("http://localhost:8080/oauth2/token").build();

		OAuth2AccessToken accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, "test-access-token",
				Instant.now(), Instant.now().plusSeconds(3600));

		OAuth2AuthorizedClient authorizedClient = new OAuth2AuthorizedClient(clientRegistration, "mcp-server",
				accessToken);
