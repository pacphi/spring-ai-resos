...
public class ResOsConfig {
	...
	public OAuth2AuthorizedClientManager authorizedClientManager(
		...
		// Use AuthorizedClientServiceOAuth2AuthorizedClientManager for
		// service-to-service
	...
	@Bean
	public RestClient resosRestClient(ObjectProvider<OAuth2AuthorizedClientManager> authorizedClientManagerProvider) {
		...
		RestClient.Builder builder = RestClient.builder().baseUrl(apiEndpoint).requestFactory(requestFactory)
				.requestInterceptor(logRequestInterceptor()).requestInterceptor(logResponseInterceptor());
	...
	private ClientHttpRequestInterceptor logRequestInterceptor() {
		return (request, body, execution) -> {
			...
			request.getHeaders().forEach((name, values) -> values.forEach(value -> log.debug("Request Header: {}={}",
					name, name.equalsIgnoreCase("Authorization") ? "[REDACTED]" : value)));
	...
	private ClientHttpRequestInterceptor oauth2Interceptor(OAuth2AuthorizedClientManager authorizedClientManager) {
		return (request, body, execution) -> {
			try {
				...
				org.springframework.security.oauth2.client.OAuth2AuthorizeRequest authorizeRequest = org.springframework.security.oauth2.client.OAuth2AuthorizeRequest
						.withClientRegistrationId("mcp-server").principal("mcp-server") // Service account principal
						.build();

				// Get authorized client (with token)
				org.springframework.security.oauth2.client.OAuth2AuthorizedClient authorizedClient = authorizedClientManager
	...
	public DefaultApi defaultApi(RestClient resosRestClient) {
		HttpServiceProxyFactory factory = HttpServiceProxyFactory.builderFor(RestClientAdapter.create(resosRestClient))
				.build();
