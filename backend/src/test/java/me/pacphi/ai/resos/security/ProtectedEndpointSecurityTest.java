...
/**
 * Integration tests for protected endpoint security. Tests scope-based and
...
class ProtectedEndpointSecurityTest {
	...
	void shouldAllowReadAccessWithReadScope() throws Exception {
		...
		// Then: Returns 200 OK
		mockMvc.perform(get("/api/v1/resos/customers")).andExpect(status().isOk());
	}
	...
	void shouldAllowAnyAuthenticatedAccess() throws Exception {
		...
		// Then: Returns 200 OK (anyRequest().authenticated() allows any authenticated
		// user)
	...
	void shouldDenyAccessWithoutAuthentication() throws Exception {
		...
		mockMvc.perform(get("/api/v1/resos/customers")).andExpect(status().isUnauthorized());
	}
	...
	void shouldDenyAccessToCustomerByIdWithOnlyReadScope() throws Exception {
		...
		mockMvc.perform(get("/api/v1/resos/customers/" + testId)).andExpect(status().isNotFound()); // 404 because
																									// customer doesn't
																									// exist, but auth
																									// passed
	}
	...
	void shouldAuthenticateSuccessfullyDespiteUnimplementedEndpoint() throws Exception {
		...
		// Then: Authentication succeeds (passed security), endpoint throws
		// UnsupportedOperationException
		// This verifies the security layer works even though business logic isn't
		// implemented
		try {
	...
	void shouldDenyAnonymousAccessToBookings() throws Exception {
		...
		// Then: Returns 401 Unauthorized
		mockMvc.perform(get("/api/v1/resos/bookings/available-dates")).andExpect(status().isUnauthorized());
	}
	...
	void shouldAllowAccessToAllResourcesWithFullScopes() throws Exception {
		...
		mockMvc.perform(get("/api/v1/resos/tables")).andExpect(status().isOk());
	}
}
