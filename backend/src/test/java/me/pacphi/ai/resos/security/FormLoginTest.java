...
/**
 * Integration tests for form-based login authentication. Tests the login page,
 * form authentication, logout, and session management.
 */
@SpringBootTest
@ActiveProfiles("test")
...
class FormLoginTest {
	...
	void shouldAuthenticateValidCredentials() throws Exception {
		...
		mockMvc.perform(formLogin("/login").user(username).password(password)).andExpect(authenticated())
	...
	void shouldRejectInvalidCredentials() throws Exception {
		...
		mockMvc.perform(formLogin("/login").user(username).password(wrongPassword)).andExpect(unauthenticated())
	...
	void shouldRejectNonExistentUser() throws Exception {
		...
		mockMvc.perform(formLogin("/login").user(nonExistentUser).password(password)).andExpect(unauthenticated())
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/login?error"));
	}
	...
	void shouldAccessProtectedResourceAfterLogin() throws Exception {
		...
		// Note: This test uses Spring Security Test's formLogin() which automatically
		// sets up the session
		mockMvc.perform(formLogin("/login").user("admin").password("admin123")).andExpect(authenticated());
