...
/**
 * Helper utility for OAuth2-related test operations. Provides methods to obtain
 * tokens, decode JWTs, and build authenticated requests.
 */
public class OAuth2TestHelper {
	 ...
	 *
	 * @param tokenEndpoint
	 *            Full token endpoint URL (e.g., http://localhost:8080/oauth2/token)
	 * @param clientId
	 *            OAuth2 client ID
	 * @param clientSecret
	 ...
	 */
	public static String obtainClientCredentialsToken(String tokenEndpoint, String clientId, String clientSecret,
			String... scopes) {
		...
		try {
			String response = restClient.post().uri(tokenEndpoint).contentType(MediaType.APPLICATION_FORM_URLENCODED)
	...
	/**
	 * Decode JWT and extract claims (header + payload). Note: This does NOT
	 * validate the signature, it's for inspection only.
	 *
	 * @param token
	 *            JWT token string
	 ...
	 */
	public static MockHttpServletRequestBuilder withBearerToken(MockHttpServletRequestBuilder requestBuilder,
			String token) {
	 ...
	 *
	 * @param jwtPayload
	 *            Decoded JWT payload (JSON string)
	 * @param claimName
	 ...
	 * Note: This is a placeholder - actual implementation would require JWT
	 * library.
	 *
	 * @return A sample expired token string
	 */
	public static String createExpiredToken() {
		// This would require a JWT library like nimbus-jose-jwt to create a properly
		// signed token
