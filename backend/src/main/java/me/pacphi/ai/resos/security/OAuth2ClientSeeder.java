...
/**
 * Seeds OAuth2 registered clients for dev and test profiles. Runs after main
 * DataSeeder to ensure user/authority tables are populated.
 */
@Component
...
public class OAuth2ClientSeeder implements CommandLineRunner {
	...
	public OAuth2ClientSeeder(JdbcTemplate jdbcTemplate, PasswordEncoder passwordEncoder,
	...
	private void seedMcpServerClient() {
		RegisteredClient mcpServer = RegisteredClient.withId(UUID.randomUUID().toString()).clientId(mcpServerClientId)
				.clientIdIssuedAt(Instant.now()).clientSecret(passwordEncoder.encode(mcpServerClientSecret))
				.clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC)
				.authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS).scope("backend.read")
				.scope("backend.write")
	...
	private void seedMcpClientClient() {
		RegisteredClient mcpClient = RegisteredClient.withId(UUID.randomUUID().toString()).clientId("mcp-client")
				.clientIdIssuedAt(Instant.now()).clientSecret(passwordEncoder.encode("mcp-client-secret"))
				.clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC)
				.authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS).scope("mcp.read").scope("mcp.write")
				.clientSettings(ClientSettings.builder().requireAuthorizationConsent(false).build())
				.tokenSettings(TokenSettings.builder().accessTokenTimeToLive(Duration.ofHours(1)).build()).build();
	...
	private void seedFrontendAppClient() {
		...
		RegisteredClient frontendApp = RegisteredClient.withId(UUID.randomUUID().toString())
				.clientId(frontendAppClientId).clientIdIssuedAt(Instant.now())
				// Public client - no client secret
				.clientAuthenticationMethod(ClientAuthenticationMethod.NONE)
				.authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
				.authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN).redirectUri(redirectUri)
				.redirectUri(authorizedUri).postLogoutRedirectUri(frontendPostLogoutRedirectUrl).scope("openid")
				.scope("profile").scope("email").scope("chat.read").scope("chat.write")
				.clientSettings(ClientSettings.builder().requireAuthorizationConsent(false) // Skip consent for dev
