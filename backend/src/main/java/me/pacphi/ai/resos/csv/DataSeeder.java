...
public class DataSeeder implements CommandLineRunner {
	...
	public DataSeeder(DataSource dataSource, RepositoryResolver repositoryResolver, CsvFileProcessor csvFileProcessor,
			CsvResourceLoader resourceLoader, ListableBeanFactory beanFactory) {
	...
	void initializeMappers() {
		...
		this.entityMappers = beanFactory.getBeansWithAnnotation(CsvEntityMapper.class).values().stream()
				.filter(bean -> bean instanceof EntityMapper).map(bean -> (EntityMapper<?>) bean)
				.collect(Collectors.toMap(mapper -> mapper.getClass().getAnnotation(CsvEntityMapper.class).value(),
						Function.identity()));
	}
