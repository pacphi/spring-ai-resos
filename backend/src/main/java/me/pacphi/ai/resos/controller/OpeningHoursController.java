...
public class OpeningHoursController {
	...
	/**
	 * GET /opening-hours : List opening hours Retrieve a list of opening hours
	 *
	 * @return List of opening hours (status code 200)
	 */
	@Operation(operationId = "openingHoursGet", summary = "List opening hours", description = "Retrieve a list of opening hours", responses = {
			@ApiResponse(responseCode = "200", description = "List of opening hours", content = {
					@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = OpeningHours.class)))})}, security = {
							@SecurityRequirement(name = "basicAuth")})
	@GetMapping(value = "/opening-hours", produces = {"application/json"})
	public ResponseEntity<List<OpeningHours>> openingHoursGet() {
		List<OpeningHours> result = StreamSupport.stream(openingHoursRepository.findAll().spliterator(), false)
				.map(OpeningHoursEntity::toPojo).toList();
		return ResponseEntity.ok(result);
	}

	/**
	 * GET /opening-hours/{id} : Get opening hours Retrieve specific opening hours
	 * by ID
	 *
	 * @param id
	 *            (required)
	 * @return Opening hours details (status code 200) or Opening hours not found
	 *         (status code 404)
	 */
	@Operation(operationId = "openingHoursIdGet", summary = "Get opening hours", description = "Retrieve specific opening hours by ID", responses = {
			...
			@ApiResponse(responseCode = "404", description = "Opening hours not found")}, security = {
					@SecurityRequirement(name = "basicAuth")})
	...
	public ResponseEntity<OpeningHours> openingHoursIdGet(
		...
		try {
			...
			return optionalEntity.map(openingHoursEntity -> ResponseEntity.ok(openingHoursEntity.toPojo()))
