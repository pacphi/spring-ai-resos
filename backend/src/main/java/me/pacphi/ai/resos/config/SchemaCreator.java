 ...
 * <li>In JAR: writes to ApplicationTemp directory (consistent across
 * restarts)</li>
 * <li>Copies patch files from classpath to temp directory for JAR
 * execution</li>
 * <li>Sets system property "liquibase.changelog.dir" for
 * LiquibaseCustomizer</li>
 * </ul>
 */
@Profile({"dev", "test"})
@Component
public class SchemaCreator {
	...
	/**
	 * System property key for communicating changelog directory to
	 * LiquibaseCustomizer. When set, indicates JAR execution mode requiring
	 * file-based changelog loading.
	 */
	...
	public SchemaCreator(@Value("${app.entity.base-package}") String entityBasePackage,
	...
	/**
	 * Determines the target path for changelog generation. Uses ApplicationTemp for
	 * JAR execution to ensure consistent directory across application restarts
	 * (important for Liquibase change tracking).
	 */
	...
	/**
	 * Gets a consistent temporary directory for JAR execution. Uses Spring Boot's
	 * ApplicationTemp which provides: - Consistent location across application
	 * restarts - Application-specific isolation
	 */
	...
	/**
	 * Copies patch files from classpath (inside JAR) to temp directory. This is
	 * necessary because the master changelog uses relative paths (patches/xxx.yml)
	 * which must exist on the filesystem.
	 */
	...
	private void updateMasterChangelog(File generatedDir, Path targetPath) throws IOException {
		...
		// Include generated entity changelogs (security entities are now part of entity
		// scanning)
		...
		try {
			org.springframework.core.io.Resource[] patchResources = new org.springframework.core.io.support.PathMatchingResourcePatternResolver()
					.getResources("classpath:db/changelog/patches/*.yml");
...
class ChangesetGenerator {
	...
	public void generateChangesets(String basePackage, String outputPath) throws IOException {
		...
		for (Class<?> entity : orderedEntities) {
			if (!this.processedEntities.contains(entity) && this.hasIdField(entity)) {
				...
				logger.debug("Skipping entity {} because it either lacks an @Id field or has already been processed.",
						entity.getName());
			}
		}
	}
	...
	private void processEntity(Class<?> entity, String outputPath) throws IOException {
		...
		String fileName = String.format("%d_%s_init.yml", currentTimestamp, getTableName(entity));
	...
	private Map<String, Object> generateChangeset(Class<?> entity, long timestamp) {
		...
		String changesetId = String.format("%d_%s_init", timestamp, tableName);
	...
	private Map<String, Object> createIdColumn() {
		...
		String defaultValue = databaseType.equals("POSTGRESQL") ? "gen_random_uuid()" : "random_uuid()";
		...
		for (Field field : entity.getDeclaredFields()) {
			if (field.isAnnotationPresent(MappedCollection.class)) {
				...
				if (!ann.keyColumn().isEmpty()) {
					...
					keyColumn.put("constraints", Collections.singletonMap("nullable", false));
