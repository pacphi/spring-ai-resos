...
/**
 * Customizes the auto-configured SpringLiquibase bean to use temp directory
 * changelogs when running from JAR (Docker containers).
 * <p>
 * This BeanPostProcessor intercepts SpringLiquibase bean initialization and
 * redirects it to use file-based changelogs from the temp directory when the
 * system property {@link SchemaCreator#CHANGELOG_DIR_PROPERTY} is set.
 * <p>
 * <b>How it works:</b>
 * <ol>
 * <li>SchemaCreator generates changelogs and sets system property</li>
 * <li>This customizer detects the property during SpringLiquibase
 * initialization</li>
 * <li>Redirects Liquibase to use file: prefix and FileSystemResourceLoader</li>
 * </ol>
 * <p>
 * <b>Important:</b> This relies on SchemaCreator's @PostConstruct running
 * before SpringLiquibase is initialized. This ordering is ensured by
 * LiquibaseConfiguration which uses @DependsOn annotation.
 *
 * @see SchemaCreator
		...
		if (tempDirPath != null) {
			...
			if (!changelogFile.toFile().exists()) {
				logger.warn("Temp changelog file does not exist yet: {}. "
						+ "This may indicate SchemaCreator hasn't run. " + "Ensure @DependsOn ordering is correct.",
						changelogFile);
