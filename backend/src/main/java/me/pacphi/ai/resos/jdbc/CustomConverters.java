 ...
 * <li>URI - to avoid Java module accessibility issues (java.net.URI private
 * constructor)</li>
 * </ul>
 */
@Configuration
public class CustomConverters extends AbstractJdbcConfiguration {
	...
	protected List<?> userConverters() {
		return Arrays.asList(new StringToOffsetDateTimeConverter(), new OffsetDateTimeToStringConverter(),
				new StringToUriConverter(), new UriToStringConverter());
	}

	/**
	 * Converts a String to URI when reading from database. This avoids the
	 * InaccessibleObjectException that occurs when Spring Data JDBC tries to access
	 * java.net.URI's private constructor without --add-opens.
	 */
	@ReadingConverter
