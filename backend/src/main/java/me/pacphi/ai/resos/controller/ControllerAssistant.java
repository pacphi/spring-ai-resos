...
class ControllerAssistant {
	...
	private static final List<String> BINARY_OPERATORS = List.of("=", "!=", ">", ">=", "<", "<=", "LIKE");

	// Unary operators (no value needed)
	private static final List<String> UNARY_OPERATORS = List.of("IS NOT NULL", // Must be before IS NULL for regex
																				// matching
			"IS NULL");
	...
	static String sanitizeCustomQuery(String customQuery, List<String> allowedFields) {
		...
		var sanityCheckRegex = "^\\s*" + conditionPattern + "(\\s+(AND|OR)\\s+" + conditionPattern + ")?\\s*$";
